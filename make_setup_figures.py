"""Paper-ready figures: layer-averaged learned cross-person edge multipliers,
one figure per training setup (scratch, pt, scratch+norm, pt+norm), each with
4class and 8class side by side, averaged over seeds 42-44.

All figures share ONE color scale (computed over every setup) so they are
directly comparable. Saves output/edge_importance_<setup>.{png,svg,pdf}."""
...
# setup slug in the run-dir name -> (file suffix, human title)
SETUPS = [
    ("full_scratch_s",      "scratch",      "scratch"),
    ("full_pt_s",           "pt",           "pretrained"),
    ("full_scratch_norm_s", "scratch_norm", "scratch + norm"),
    ("full_pt_norm_s",      "pt_norm",      "pretrained + norm"),
]
...
def seed_mean_cross(pattern):
    runs = sorted(glob.glob(pattern))
    if not runs:
        sys.exit(f"no runs match {pattern}")
    print(f"  {len(runs)} runs: " + ", ".join(os.path.basename(r) for r in runs))
    return np.nanmean(np.stack([mean_cross(r) for r in runs]), axis=0)

# gather everything first so all figures share one color scale
data = {}
for slug, suffix, title in SETUPS:
    print(f"{title}:")
    data[suffix] = [
        (f"(a) 4-class: technique ({title})",
         seed_mean_cross(f"output/*_4class_{slug}4?_training")),
        (f"(b) 8-class: technique x point ({title})",
         seed_mean_cross(f"output/*_8class_{slug}4?_training")),
    ]

span = max(np.nanmax(np.abs(c - 1.0)) for panels in data.values() for _, c in panels)
norm = TwoSlopeNorm(vcenter=1.0, vmin=1.0 - span, vmax=1.0 + span)
cmap = plt.get_cmap("coolwarm")

for slug, suffix, title in SETUPS:
    fig, axes = plt.subplots(1, 2, figsize=(13.6, 6.0))
    for k, (ax, (panel_title, cross)) in enumerate(zip(axes, data[suffix])):
    ...
    for ext, kw in [("png", {"dpi": 200}), ("svg", {}), ("pdf", {})]:
        out = f"output/edge_importance_{suffix}.{ext}"
